...
*/
 buffer_status cb_init(cb *ptr, uint8_t buffer_length)
  ...
  if(ptr==NULL || buffer_length <=0)					/*Null pointer check*/
    {
    status=null_ptr;
    }
  else
    ...
    if(NULL==(ptr->buffer))
      ...
      {      
      ptr->buffer= (uint8_t*)malloc(sizeof(uint8_t)*buffer_length);	/*Allocating the buffer with the size specified*/ 	
      ptr->size= buffer_length;						/*Initialising the buffer size with the buffer length specified */
      ptr->count=0;							/*Initialising the count to be zero*/
      ptr->head=ptr->buffer;						/*Making the head pointer point to the buffer*/
      ptr->tail=ptr->buffer;						/*Making the tail pointer point to the buffer*/
      status=cb_init_success;						/*Returning the status as success*/
      }
    }
  return status;
  }
...
the function returns cb_success status on successfull addition of the element.
*/
...
the function returns cb_success status on successfull deletion of the element.
*/
